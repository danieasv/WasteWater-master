...
package Examples "Demonstration examples of the components of the ASM3 library"
  ...
  class SmallPlant "Small WWTP Configuration"
    ...
    Modelica.Blocks.Sources.Constant Constant1 annotation (Placement(
          transformation(extent={{-84,-86},{-64,-66}})));
    ...
    Modelica.Blocks.Sources.CombiTimeTable CombiTableTime(
      fileName=Modelica.Utilities.Files.loadResource("modelica://WasteWater/Resources/ASM3/Inf_dry.txt"),
      table=[0,0; 1,1],
...
"));
  ...
  class BenchPlant "COST Benchmark WWTP Configuration"
    ...
    ASM3.deni tank2(K_A_NH=0.4, b_A_O_T=0.1) annotation (Placement(
          transformation(extent={{-48,22},{-28,42}})));
    ASM3.deni tank1(K_A_NH=0.4, b_A_O_T=0.1) annotation (Placement(
          transformation(extent={{-76,22},{-56,42}})));
    ASM3.mixer3 mixer annotation (Placement(transformation(extent={{-104,22},{
              -84,42}})));
    ASM3.WWSource WWSource annotation (Placement(transformation(extent={{-88,78},
              {-68,98}})));
    ...
    Modelica.Blocks.Sources.Step OxygenSetPoint(height=2)
      annotation (Placement(transformation(extent={{40,40},{58,58}})));
    ...
    Modelica.Blocks.Sources.Constant Temperature(k=15)
      annotation (Placement(transformation(extent={{-94,50},{-82,62}})));
    Modelica.Blocks.Sources.CombiTimeTable CombiTableTime(
      fileName=Modelica.Utilities.Files.loadResource("modelica://WasteWater/Resources/ASM3/Inf_rain.txt"),
      table=[0,0; 1,1],
    ...
    WasteWater.ASM3.sensor_COD sensor_COD1 annotation (Placement(transformation(
            extent={{76,18},{92,34}})));
  equation
    connect(divider.Out1, Settler.Feed) annotation (Line(points={{39.9,7},{39.9,
            9.4},{48,9.4}}));
    connect(tank5.Out, divider.In) annotation (Line(points={{14,4},{16,4},{16,
            4.3},{19.9,4.3}}));
    ...
    connect(mixer.Out, tank1.In) annotation (Line(points={{-84,31.7},{-80,31.7},
            {-80,32},{-76,32}}));
    ...
    connect(divider.Out2, RecyclePump.In) annotation (Line(points={{40,2.2},{44,
            2.2},{44,-8.7},{-74,-8.7}}));
    connect(RecyclePump.Out, mixer.In3) annotation (Line(points={{-94,-14.9},{
            -108,-14.9},{-108,26.6},{-103.9,26.6},{-103.9,27.1}}));
    ...
    connect(Settler.Return, ReturnPump.In) annotation (Line(points={{55,-1.6},{
            55,-22.7},{36,-22.7}}));
    connect(ReturnPump.Out, mixer.In2) annotation (Line(points={{16,-28.9},{
            -112,-28.9},{-112,31.6},{-104.1,31.6}}));
    connect(Settler.Waste, WastePump.In) annotation (Line(points={{61,-1.6},{61,
            -27.3},{70,-27.3}}));
    connect(WastePump.Out, WasteSludge.In) annotation (Line(points={{90,-21.1},
    ...
    connect(Temperature.y, tank2.T)
      annotation (Line(points={{-81.4,56},{-48,56},{-48,36}}, color={0,0,255}));
...
"));
  ...
  model ComplexPlant "Complex ASM2d WWTP"
    ...
    Modelica.Blocks.Sources.CombiTimeTable CombiTableTime1(
      fileName=Modelica.Utilities.Files.loadResource("modelica://WasteWater/Resources/ASM3/drysim130303.txt"),
      table=[0,0; 1,1],
      columns=integer(({2})),
